...
class Indoor_dataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		if np.array(item['lightmap']).max() != 0:
			...
			lightmap = lightmap.convert('RGB')	# for lightmap
			scribble = lightmap.convert('RGB')	# for lightmap
			...
			lightmap = self.conditioning_image_transforms(lightmap)	# for lightmap
			scribble = self.conditioning_image_transforms(scribble)	# for lightmap
			...
			# source = shading
			source = torch.cat((normal, scribble), dim=0)	# for lightmap
			# # depthe + mask + normal
			# DM = depth * mask
			# source = torch.cat((normal, depth, mask, DM), dim=0)

			control_target = torch.cat((normal, lightmap), dim=0)	# for lightmap
			# control_target = torch.cat((normal, depth, mask, DM), dim=0)	# depth + mask + normal

			prompt = self.tokenizer(
				prompt, max_length=self.tokenizer.model_max_length-4, padding="max_length", truncation=True, return_tensors="pt"
			)

			return dict(pixel_values=i_phys, input_ids=prompt.input_ids, albedo=albedo, intensity=intensity, color=color, conditioning_pixel_values=source, control_target=control_target, ori_img=ori, normal=normal, depth=depth, mask=mask, lightmap=lightmap)
