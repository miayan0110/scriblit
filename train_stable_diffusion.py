...
from data_loader import Indoor_dataset, shading2scrib
...
def log_validation(
	...
	for validation_prompt, validation_image in zip(validation_prompts, validation_images):
		...
		validation_shading = shading2scrib(Image.open(validation_image.replace("normal", "shading"))).convert("RGB")
		...
		for _ in range(args.num_validation_images):
			with inference_ctx:
				image = pipeline(
					validation_prompt, (validation_normal, validation_shading), num_inference_steps=20, generator=generator, albedo_latents= albedo_latents
				).images[0]
...
def replace_unet_conv_in(unet):
	...
	unet.down_blocks[0].requires_grad_(True)
	...
	for epoch in range(first_epoch, args.num_train_epochs):
		for step, batch in enumerate(train_dataloader):
			with accelerator.accumulate(controlnet):
				...
				albedo_noisy_latents = noise_scheduler.add_noise(albedo_latents, albedo_noise, timesteps)
