...
from dataloader_ex3 import Indoor_dataset, shading2scrib
	...
	for epoch in range(first_epoch, args.num_train_epochs):
		for step, batch in enumerate(train_dataloader):
			with accelerator.accumulate(controlnet):
				...
				del pred_z0, diff, denom, per_sample, pseudo_gt, mask
				...
				loss = latent_loss + image_loss + 10 * cond_loss
			...
			if accelerator.sync_gradients:
				...
				if accelerator.is_main_process:
					if global_step % args.checkpointing_steps == 0:
						...
						write_tb_log_source(masked_pseudo_gt, 'masked_pseudo_gt', n_img, log_writer, global_step)
						write_tb_log(batch["conditioning_pixel_values"][:,:3,:,:], 'normal', n_img, log_writer, global_step)
						write_tb_log(batch["conditioning_pixel_values"][:,-1,:,:], 'depth*mask', n_img, log_writer, global_step)
						write_tb_log(batch["control_target"][:,-1,:,:], 'depth*mask (gt)', n_img, log_writer, global_step)
