...
from dataloader_ex5 import Indoor_dataset, shading2scrib
...
def parse_args(input_args=None):
	...
	parser.add_argument(
        "--hf_repo_id",
        type=str,
        default='Miayan/project',
        help="HF repo id, e.g. 'Miayan/project'.",
    )
	parser.add_argument(
        "--hf_version",
        type=str,
        default=None,
        help="Subdir in the repo that contains checkpoint, e.g. 'train_ex6'.",
    )
	parser.add_argument(
		"--dataset",
		type=str,
...
def main(args):
	...
	if args.hf_version is not None:
		unet = UNet2DConditionModel.from_pretrained(args.hf_repo_id, subfolder=f'{args.hf_version}/checkpoint-235000/custom_unet', torch_dtype=torch.float32)
		cond_encoder = CustomEncoder.from_pretrained(args.hf_repo_id, subfolder=f'{args.hf_version}/checkpoint-235000/custom_encoder', torch_dtype=torch.float32)
		controlnet = ControlNetModel.from_pretrained(args.hf_repo_id, subfolder=f'{args.hf_version}/checkpoint-235000/controlnet', torch_dtype=torch.float32)
	else:
