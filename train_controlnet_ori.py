		...
		for step, batch in enumerate(train_dataloader):
			with accelerator.accumulate(controlnet):
				...
				with torch.no_grad():
					...
					iphys_latents = vae.encode(batch["pseudo_gt"].to(dtype=weight_dtype)).latent_dist.sample()
					iphys_latents = iphys_latents * vae.config.scaling_factor
				...
				phys_loss = F.mse_loss(pred_z0.float(), iphys_latents.float(), reduction="mean")

				# Get the target for loss depending on the prediction type
				if noise_scheduler.config.prediction_type == "epsilon":	
					target = noise
				elif noise_scheduler.config.prediction_type == "v_prediction":
					target = noise_scheduler.get_velocity(iphys_latents, noise, timesteps)
				else:
				...
				loss = latent_loss + image_loss + 5 * phys_loss
				...
				del cat_latents, noisy_latents, albedo_latents, latents, iphys_latents
				torch.cuda.empty_cache()
			...
			if accelerator.sync_gradients:
				...
				if accelerator.is_main_process:
					if global_step % args.checkpointing_steps == 0:
						...
						write_tb_log_source(batch['pseudo_gt'], 'pseudo_GT', n_img, log_writer, global_step)
