...
def load_models(path, model_dir='/mnt/HDD7/miayan/paper/scriblit/dataset/iid', stage=4, device='cuda', compiled=False, chroma_dpt=False, alb_residual=False):
    ...
    if isinstance(path, str):
        if path in ['paper_weights', 'rendered_only']:
            ...
            combined_dict = torch.hub.load_state_dict_from_url(V1_DICT[path], map_location=device, progress=True, model_dir="./iid")
            stage = 1
            ...
            ord_state_dict = torch.hub.load_state_dict_from_url(base_url + 'stage_0.pt' , map_location=device, progress=True, model_dir="./iid")
            iid_state_dict = torch.hub.load_state_dict_from_url(base_url + 'stage_1.pt' , map_location=device, progress=True, model_dir="./iid")
            col_state_dict = torch.hub.load_state_dict_from_url(base_url + 'stage_2.pt' , map_location=device, progress=True, model_dir="./iid")
            alb_state_dict = torch.hub.load_state_dict_from_url(base_url + 'stage_3.pt' , map_location=device, progress=True, model_dir="./iid")
            dif_state_dict = torch.hub.load_state_dict_from_url(base_url + 'stage_4.pt' , map_location=device, progress=True, model_dir="./iid")
        elif path == 'v2.1':
