...
class Indoor_dataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		if np.array(item['lightmap']).max() != 0:
			...
			return dict(pixel_values=i_phys, input_ids=prompt.input_ids, albedo=albedo, intensity=intensity, color=color, conditioning_pixel_values=source, control_target=control_target, ori_img=ori, normal=normal, depth=depth)
