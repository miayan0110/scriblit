...
def parse_args(input_args=None):
	...
	parser.add_argument(
		"--dataset",
		type=str,
		default="/mnt/HDD7/miayan/paper/relighting_datasets/lsun_test"
	)
...
# def pred_x0_latents(noise_scheduler, model_pred, x_t, t):
#     """ v1
#     model_pred: UNet 的輸出 (B,4,H,W)，對應 scheduler.config.prediction_type
#     x_t:       noisy latents (B,4,H,W)
#     t:         (B,) 每張圖的 timestep
#     return:    \hat{x}_0 (B,4,H,W)
#     """
#     device = x_t.device
#     alpha_bar = noise_scheduler.alphas_cumprod.to(device)[t.long()].view(-1, 1, 1, 1)
#     sqrt_ab = alpha_bar.sqrt()
#     sqrt_1mab = (1.0 - alpha_bar).sqrt()

#     if noise_scheduler.config.prediction_type == "epsilon":
#         x0 = (x_t - sqrt_1mab * model_pred) / sqrt_ab
#     elif noise_scheduler.config.prediction_type == "v_prediction":
#         x0 = sqrt_ab * x_t - sqrt_1mab * model_pred
#     else:
...
def main(args):
	...
	# ---- 只讓 cross-attention / transformer blocks 可學 v4 ----
	...
	# unet.conv_in.requires_grad_(True)	# v3
	...
	for epoch in range(first_epoch, args.num_train_epochs):
		for step, batch in enumerate(train_dataloader):
			with accelerator.accumulate(controlnet):
				...
				I_input = batch["pixel_values"]
				input_latents = vae.encode(I_input.to(dtype=weight_dtype)).latent_dist.sample()
				input_latents = input_latents * vae.config.scaling_factor

				I_phys = batch["pseudo_gt"]
				phys_latents = vae.encode(I_phys.to(dtype=weight_dtype)).latent_dist.sample()
				phys_latents = phys_latents * vae.config.scaling_factor

				I_alb = batch["albedo"]
				albedo_latents = vae.encode(I_alb.to(dtype=weight_dtype)).latent_dist.sample()
				albedo_latents = albedo_latents * vae.config.scaling_factor

				## Get the text embedding for conditioning
				# 這邊我覺得還是應該放normal+mask，目前是normal+lightmap
				controlnet_image = batch["conditioning_pixel_values"].to(dtype=weight_dtype)

				## Get color and intensity embedding for conditioning
				i1 = batch['intensity'].to(input_latents.device)
				# i2 = torch.clamp(i1_safe + 0.5 * (1.0 - i1_safe), 0.0, 1.0)
				i0 = torch.zeros_like(i1)
				color = batch['color'].to(input_latents.device)

				# run one of cycle branch
				def run_branch(in_latents, alb_latents, intensity, color):
					# sample noise
					in_noise = torch.randn_like(in_latents)
					alb_noise = torch.randn_like(alb_latents)
					bsz = in_latents.shape[0]

					# sample random timestep for each image
					in_timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=in_latents.device)
					alb_timesteps = torch.randint(200, 201, (bsz,), device=in_latents.device)

					# add noise to latents
					in_latents = noise_scheduler.add_noise(in_latents, in_noise, in_timesteps)
					alb_latents = noise_scheduler.add_noise(alb_latents, alb_noise, alb_timesteps)
					...
					cat_latents = torch.cat([in_latents, alb_latents], dim=1).float()  # [B, 8, h, w]
					...
					z0 = pred_x0_latents(noise_scheduler, model_pred, in_latents, in_timesteps)
					I = vae.decode(z0 / vae.config.scaling_factor).sample

					# Get the target for loss depending on the prediction type
					if noise_scheduler.config.prediction_type == "epsilon":	
						target  = in_noise
					elif noise_scheduler.config.prediction_type == "v_prediction":
						target = noise_scheduler.get_velocity(in_latents, in_noise, in_timesteps)
					else:
						raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")
					
					return model_pred, controlnet_cond_recon, z0, I, target
				
				
				# on
				pred_i1, ctrlrec_i1, z0_i1, I_relit, target_i1 = run_branch(input_latents, albedo_latents, i1, color)

				latent_loss_on  = F.mse_loss(pred_i1.float(), target_i1.float(), reduction="mean")
				L_phys = F.l1_loss(z0_i1, phys_latents)
				control_target = batch["control_target"].to(dtype=weight_dtype)
				controlnet_cond_loss = F.mse_loss(ctrlrec_i1, control_target)

				loss_on = latent_loss_on + L_phys + controlnet_cond_loss
				accelerator.backward(loss_on)

				# 存 scalar/圖到安全位置，再釋放 GPU 張量
				z0_i1_log = z0_i1.detach()
				I_relit_log = I_relit.detach()
				latent_loss_on_s = latent_loss_on.detach()
				L_phys_s         = L_phys.detach()
				controlnet_cond_loss_s     = controlnet_cond_loss.detach()
				del pred_i1, ctrlrec_i1, z0_i1, I_relit, target_i1, latent_loss_on, L_phys, controlnet_cond_loss
				torch.cuda.empty_cache()  # 若之前常爆，這裡放一次；穩了可拿掉
				
				# off
				pred_i0, ctrlrec_i0, z0_i0, I_rec, target_i0 = run_branch(z0_i1_log, albedo_latents, i0, color)

				latent_loss_off = F.mse_loss(pred_i0.float(), target_i0.float(), reduction="mean")
				L_rec = F.l1_loss(z0_i0, input_latents)
				L_change = F.l1_loss(torch.abs(I_relit_log - I_input), torch.abs(I_relit_log - I_rec))

				loss_off = latent_loss_off + L_rec + L_change
				accelerator.backward(loss_off)

				latent_loss_off_s = latent_loss_off.detach()
				L_rec_s           = L_rec.detach()
				L_change_s    = L_change.detach()

				del pred_i0, ctrlrec_i0, z0_i0, target_i0, latent_loss_off, L_rec, L_change

				## 未來有需要再說
				# pred_i2, ctrlrec_i2, z0_i2, I_relit_strong = run_branch(i2, color)
				...
				loss         = (loss_off.detach() + loss_on.detach())                         # 總 loss
				latent_loss  = (latent_loss_off_s + latent_loss_on_s)        # 兩邊 latent 項相加

				total_loss += loss.detach().item()
				total_latent_loss += latent_loss.detach().item()
				total_controlnet_cond_loss += controlnet_cond_loss_s.detach().item()
				total_L_rec += L_rec_s.detach().item()
				total_L_change += L_change_s.detach().item()
				total_L_phys += L_phys_s.detach().item()
			...
			if accelerator.sync_gradients:
				...
				if accelerator.is_main_process:
					if global_step % args.checkpointing_steps == 0:
						...
						log_writer.add_scalars('Image Loss', {'train': total_controlnet_cond_loss/args.checkpointing_steps}, global_step)
						log_writer.add_scalars('Reconstruction Loss', {'train': total_L_rec/args.checkpointing_steps}, global_step)
						log_writer.add_scalars('Change Loss', {'train': total_L_change/args.checkpointing_steps}, global_step)
						log_writer.add_scalars('Physical Loss', {'train': total_L_phys/args.checkpointing_steps}, global_step)
						total_loss = 0
						total_latent_loss = 0
						total_controlnet_cond_loss = 0
						total_L_rec = 0
						total_L_change = 0
						total_L_phys = 0
						write_tb_log_source(I_relit_log, 'relighted image', n_img, log_writer, global_step)
						write_tb_log_source(I_phys, 'pseudo gt', n_img, log_writer, global_step)
						write_tb_log_source(I_rec, 'reconstruct image', n_img, log_writer, global_step)
						write_tb_log_source(batch["pixel_values"], 'img_src', n_img, log_writer, global_step)
						write_tb_log(batch["control_target"][:,3:6,:,:], 'lightmap', n_img, log_writer, global_step)
						write_tb_log(batch["conditioning_pixel_values"][:,:3,:,:], 'normal', n_img, log_writer, global_step)
						write_tb_log(batch["conditioning_pixel_values"][:,3:6,:,:], 'scribble', n_img, log_writer, global_step)
						write_tb_log_source(batch["albedo"], 'albedo', n_img, log_writer, global_step)
