...
class CustomControlNetPipeline(
	...
	def check_inputs(
		...
		if prompt is not None and prompt_embeds is not None:
			...
			raise ValueError(
				"Provide either `prompt` or `prompt_embeds`, `intensity`, and `color`. Cannot leave both `prompt` and `prompt_embeds`, `intensity`, and `color` undefined."
			)
	...
	def __call__(
		...
		**kwargs,
		...
		print(intensity.shape, color.shape)
		if intensity is not None and isinstance(intensity, torch.Tensor):
		...
		with self.progress_bar(total=num_inference_steps) as progress_bar:
			for i, t in enumerate(timesteps):
				...
				latent_model_input = torch.cat([cat_latents] * 2) if self.do_classifier_free_guidance else latents
