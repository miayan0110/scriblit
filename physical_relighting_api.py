...
class PhysicalRelightingConfig:
    def __init__(self, ori, normal, depth, out_lightmap, out_relit):
        # 基本輸入
        self.normal = normal
        self.depth = depth
        self.original = ori
    ...
    def add_mask(self, path, color=(1.0,1.0,1.0), gain=1.0):
        ...
        self.mask.append((path, color, gain))


def compute_relighting(phys_cfg):

    device = 'cuda:7' if torch.cuda.is_available() else 'cpu'
    print('Device:', device)

    # load original
    orig_img = Image.open(phys_cfg.original).convert('RGB')
    ...
    # Image.fromarray((light_gray[0].cpu().numpy()*255).astype(np.uint8)).save(phys_cfg.out_light)
    # to_img(relit).save(phys_cfg.out_relit)
